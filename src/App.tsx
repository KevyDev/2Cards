...
export default function App() {
    let [loaded, setLoaded] = useState<boolean>(false),
        [allCards, setAllCards] = useState<string[]>([]),
        [error, setError] = useState<boolean>(false),
        [currentPairs, setCurrentPairs] = useState<Pair[]>([]),
    ...
    useEffect(() => {
        fetchCards(setAllCards, setError)
    }, [])

    const loadPairs = () => setCurrentPairs(selectPairs([...allCards]))
        ...
        <main className="fixed top-0 bottom-0 left-0 right-0 grid place-content-center">
            ...
            {ended && (
                <div className={"fixed top-0 bottom-0 left-0 right-0 z-20 flex flex-col gap-4 items-center justify-center bg-white"}>
                    ...
                    <button onClick={restartGame} className="bg-blue-500 hover:bg-blue-400 duration-200 px-8 py-2 text-xl text-white rounded hover:shadow">Play again</button>
                </div>
            )}
            {currentPairs.length > 0 && (
                <ul className="grid grid-cols-4 sm:grid-cols-5 p-4 gap-2">
                    {currentPairs.map(({image, selected}: Pair, index:number) => {
                        let active:Boolean = activeCards.length > 0 && (activeCards[0] === index || activeCards[1] === index)
                        return (
                            <li
                                key={"card-" + index}
                                onClick={() => !active && !selected && activeCards.length < 2 && selectCard(index)}
